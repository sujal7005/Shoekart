...
const ProductDetails = () => {
  ...
  const [data, setData] = useState(null); // Initialize as null, not array
  ...
  const handleAddToCart = async () => {
    try {
      ...
      // Update cart size in auth context if available
      if (setAuth) {
        // Fetch updated cart size
        const cartResponse = await Axios.get("/api/v1/cart", {
          headers: { Authorization: `Bearer ${token}` }
        });
        const newCartSize = cartResponse.data?.items?.length || 0;
  ...
  // Safe calculation for average rating
    ...
    <section className="product-bg">
      <div className="prod-images-cont">
        <div className="prod-image">
          <img src={data.image || "/placeholder.png"} alt={data.name || "product"} />
        </div>
        <div className="pRow">
          <img src={data.image || "/placeholder.png"} alt="img" />
          <img src={data.image || "/placeholder.png"} alt="img" />
          <img src={data.image || "/placeholder.png"} alt="img" />
        </div>
      </div>
      <div className="prod-details-cont">
        ...
        <h3 className="pprize">
          ₹ {data.price} 
          {data.originalPrice && <span> {data.originalPrice}</span>}
        </h3>
        <div className="pStar">
        ...
        <select
          ...
          id="size"
          value={size}
          disabled={!data.sizeQuantity || data.sizeQuantity.length === 0}
          onChange={(e) => setSize(e.target.value)}
        >
          <option value="">Select Size</option>
          {data.sizeQuantity && data.sizeQuantity
        ...
        {(!data.sizeQuantity || data.sizeQuantity.length === 0) && (
        ...
        <p>{data.description || data.desc || "No description available"}</p>
        ...
        {data.ratings && <RatingContainer ratings={data.ratings} />}
      </div>
    </section>
  );
};
